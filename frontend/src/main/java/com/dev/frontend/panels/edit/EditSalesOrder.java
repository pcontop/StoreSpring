...
public class EditSalesOrder extends EditContentPanel
{
	...
	public void addRow()
	{
		...
		try
		{
			qty = Utils.parseInteger(txtQuantity.getText());
		}
		catch (Exception e)
		{
	...
	public boolean bindToGUI(Object o) {
		SalesOrderDto order = (SalesOrderDto) o;
		if (order==null){
			return false;
		}
		txtOrderNum.setText(order.getOrderNum());
		txtTotalPrice.setText(TextUtil.convert(order.getTotalPrice()));
		...
		for (ProductOrderDto productOrder: order.getProductOrders()){
			defaultTableModel.addRow(new String[] { productOrder.getProductCode(), 
					"" + productOrder.getProductQuantity() , 
					"" + productOrder.getProductPrice(), 
					"" + productOrder.getTotalPrice() });
		}				
		
		ComboBoxItem comboBoxCustomer = new ComboBoxItem(order.getCustomerCode(),null);
		txtCustomer.setSelectedItem(comboBoxCustomer);
	...
	public Object guiToObject() {
		...
		for (int i=0; i<defaultTableModel.getRowCount();i++){
			...
			.setCode((String) defaultTableModel.getValueAt(i, 0))
			.setQuantity(Utils.parseInteger((String) defaultTableModel.getValueAt(i, 1)))
			.setPrice(Utils.parseDouble((String) defaultTableModel.getValueAt(i, 2)))
			.build();
			productOrders.add(productOrderDto);			
		}
		
		ComboBoxItem customerItem = (ComboBoxItem) txtCustomer.getSelectedItem();
				
		SalesOrderDto salesOrderDto = SalesOrderDto.create()
				.setOrderNum(txtOrderNum.getText())
				.setCustomerCode(customerItem.getKey())
				.setCustomerName(customerItem.getValue())
